...
function CustomTooltip({ active, payload, label, simple }) {
    ...
    <div className="bg-white border border-slate-200 rounded-lg shadow-lg px-3 py-2.5 text-sm">
      ...
      {simple ? (
        ...
        <>
          ...
          <div className="flex justify-between gap-4"><span className="text-slate-400">25th %ile</span><span className="font-mono text-slate-600">{fmtCurrency(d.p25)}</span></div>
          <div className="flex justify-between gap-4"><span className="text-slate-400">75th %ile</span><span className="font-mono text-slate-600">{fmtCurrency(d.p75)}</span></div>
...
export default function AVFanChart({ data, simple = false, lifeExpectancyAge = null }) {
  if (!data || data.length === 0) return null;

  const allChartData = transformData(data);
  const { chartHandlers, filterData, xDomain, refArea, isZoomed, reset } = useChartZoom();
  const chartData = filterData(allChartData);

  // Axis bounds computed from full data (not zoomed slice) so Y doesn't jump on zoom
    ...
    <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-100">
      <div className="flex items-center justify-between mb-0.5">
        <div className="text-sm font-semibold text-slate-700">
          {simple ? 'How Your Account Value May Grow' : 'Account Value Projection — Fan Chart'}
        </div>
        {isZoomed && (
      ...
      {simple && (
        <div className="text-xs text-slate-400 mb-1">
          Simulated range of account values based on thousands of market scenarios
        </div>
      )}
      {!isZoomed && (
        <div className="text-xs text-slate-400 mb-2 italic">Drag on the chart to zoom in</div>
      )}
        ...
        <ResponsiveContainer width="100%" height={320}>
          <ComposedChart data={chartData} margin={{ top: 5, right: 20, bottom: 20, left: 20 }}
            ...
            <YAxis
              ...
              domain={[0, yMax]}
            />
            <Tooltip content={<CustomTooltip simple={simple} />} />
            ...
            <Area type="monotone" dataKey="outer_fill" stackId="outer"
              ...
              fill="#dbeafe" fillOpacity={0.6} name={outerBandName} />
            ...
            <Area type="monotone" dataKey="inner_fill" stackId="inner"
              ...
              fill="#93c5fd" fillOpacity={0.7} name={innerBandName} />

            {!simple && (
              <Line type="monotone" dataKey="mean"
                stroke="#2563eb" strokeWidth={2} dot={false} name="Mean" />
            )}
            <Line type="monotone" dataKey="median"
              stroke="#1d4ed8" strokeWidth={simple ? 2 : 1.5}
      ...
      <p className="text-xs text-slate-400 mt-1">
        {simple
          ? 'Hover over the chart to see values at each age. Shaded bands show the range of likely outcomes.'
          : 'Shaded bands: light = 5th–95th %ile, dark = 25th–75th %ile AV across all scenarios.'}
