...
// ---------------------------------------------------------------------------
// RIA Interest / Free Analysis CTA
// ---------------------------------------------------------------------------
function FreeAnalysisCTA() {
  ...
  const handleSubmit = async (e) => {
    ...
    setStatus('loading');
    try {
      const res = await fetch('/api/ria-interest', {
        method: 'POST',
      ...
      <div className="max-w-xl mx-auto">
        {status === 'success' ? (
          <div className="text-center">
            ...
            <h2 className="text-2xl font-black text-white mb-3">We'll be in touch.</h2>
            <p className="text-slate-400 leading-relaxed">
              Kai will follow up personally within one business day.
            </p>
          </div>
        ) : (
          <>
            <div className="text-center mb-8">
              <h2 className="text-3xl font-black text-white mb-3">Work with Us</h2>
              <p className="text-slate-400 leading-relaxed">
                When a carrier designs an income rider, they have a team of actuaries working for them.
                Your client signed that contract without one. We run the same math — on the policyholder's side.
              </p>
            </div>
            ...
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Doug Richards, CFP"
                    className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:border-blue-400 text-sm transition-colors"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">
                  What would help you most? <span className="text-slate-600 normal-case font-normal">(optional)</span>
                </label>
                <textarea
                  rows={3}
                  value={message}
                  onChange={e => setMessage(e.target.value)}
                  placeholder="e.g. I have a client with a Jackson National contract I've been struggling to evaluate..."
                  className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:border-blue-400 text-sm transition-colors resize-none"
                />
              </div>

              {status === 'error' && (
                <p className="text-amber-600 text-sm">Something went wrong — try emailing kai@annuityvoice.com directly.</p>
              )}

              <button
                type="submit"
                disabled={status === 'loading'}
                className="w-full flex items-center justify-center gap-2 px-6 py-3.5 bg-[#0052CC] text-white font-bold rounded-xl hover:bg-blue-600 disabled:opacity-60 transition-all shadow-lg shadow-blue-900/40 hover:-translate-y-0.5 disabled:translate-y-0"
              >
                {status === 'loading' ? (
              ...
              <p className="text-center text-xs text-slate-600">For financial professionals only.</p>
            </form>
          </>
        )}
      </div>
